...
func UpdateRoom_2_1()
{
	AIUpdateTrain(ObjFind(1002));
}

func ActionObject_1003()
{
	cm = PlayerGet(P_CUSTOMMOVE);
	cm++; if(cm>2) cm=0;
	
	if(cm==0)	Message0(3,2,"BACK TO DEFAULT\nENGINE MOVEMENT!");
	if(cm==1)	Message0(3,2,"ENTER DEFAULT\nSCRIPTED MOVEMENT!\nSAME AS IN ENGINE,\nBUT IT'S SCRIPTED.");
	if(cm==2)	Message0(3,2,"ENTER SHRINK MOVEMENT!\nA SPECIAL SCRITPED\nCUSTOM MOVEMENT.");
	MessagePop();
	
	PlayerSet(P_CUSTOMMOVE,cm);
}
