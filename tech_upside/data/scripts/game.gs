...
{
	ObjSetName(ObjFind(100),"BUCKET");
	ObjSetName(ObjFind(101),"FLIPPERS");
	ObjSetName(ObjFind(102),"SCUBA");
}
...
{
	AICommandScroll(1001,1);
}

func CollideObject_1000_0()
{
	AICommandScroll(1001,0);
}
...
func OutRoom_0_0()
{
	...
	if(PlayerGet(P_Y)>=roomh) // exit down
	{
		GameSet(G_UPSIDEDOWN,1);
		PlayerSet(P_Y,roomh*3-1);
		PlayerEnterJump(PlayerGet(P_DIR),DIZ_POW);
	}
}
...
{
	roomh = GameGet(G_ROOMH);
	if(PlayerGet(P_Y)>=roomh*3) // exit down
	{
		GameSet(G_UPSIDEDOWN,0);
		PlayerSet(P_Y,roomh-1);
		PlayerEnterJump(PlayerGet(P_DIR),DIZ_POW);
	}
}
