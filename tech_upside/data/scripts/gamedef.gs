...
#def GAME_TITLE			"Upside Down Demo"					// game's title (for the window title)
#def GAME_WEBSITE 		"http://www.yolkfolk.com/dizzyage"	// game's website (for the final dialog)
#def GAME_ID			"UPSD080201"						// game's id text, used in saved games - change this when you release a new version, to prevent old saved games to be loaded.
...
#def PLAYER_MAINMENUX	800		// set the player's position x in the mainmenu room (attract mode room)
#def PLAYER_MAINMENUY	40		// set the player's position y in the mainmenu room (attract mode room)
#def PLAYER_LAYER		5		// set the player's layer in the game (player gets painted last in it's layer)
#def PLAYER_BEGINX		40		// set the starting positon x of the player in map (when a new game begins)
#def PLAYER_BEGINY		88		// set the starting positon y of the player in map (when a new game begins)
...
#def ID_SCUBA			102		// set scuba item's id if you have a scuba in your game and you want to wear it as a costume for WaterPlay
...
#def G_UPSIDEDOWN		100		// if world is upsidedown
