...
#def GAME_TITLE			"WaterPlay Demo"					// game's title (for the window title)
#def GAME_WEBSITE 		"http://www.yolkfolk.com/dizzyage"	// game's website (for the final dialog)
#def GAME_ID			"WP080201"							// game's id text, used in saved games - change this when you release a new version, to prevent old saved games to be loaded.
...
#def PLAYER_MAINMENUX	0		// set the player's position x in the mainmenu room (attract mode room)
#def PLAYER_MAINMENUY	0		// set the player's position y in the mainmenu room (attract mode room)
#def PLAYER_LAYER		5		// set the player's layer in the game (player gets painted last in it's layer)
#def PLAYER_BEGINX		300		// set the starting positon x of the player in map (when a new game begins)
#def PLAYER_BEGINY		158		// set the starting positon y of the player in map (when a new game begins)
#def PLAYER_BUBBLES		12		// set this to the count of air bubble objects you placed in the map. used in WaterPlay

#def SUPPORT_WATERPLAY	1		// set to 1 if player can enter in water, swim, etc, or to 0 otherwise
#def SUPPORT_JUMPUP		1		// set to 1 if player can jump by pressing the up key, or to 0 otherwise

// WaterPlay items: scuba, flippers, and lots of air bubbles
// For more details see HandlerPlayerUpdate and PlayerCanSwim functions
#def ID_SCUBA			100		// set scuba item's id if you have a scuba in your game and you want to wear it as a costume for WaterPlay
#def ID_FLIPPERS		101		// set flipers item's id if you have any flippers in your game and you want to swim in WaterPlay (or you can set the id of the scuba if no flippers item)
#def ID_BUBBLES			9000	// set this to the first valid bubble object. the rest of the PLAYER_BUBBLES bubbles have consecutive ids. used in WaterPlay
