...
#def GAME_TITLE			"Dizzy And The Mushrooms Pie"		// game's title (for the window title)
#def GAME_WEBSITE 		"http://www.yolkfolk.com/dizzyage"	// game's website (for the final dialog)
#def GAME_ID			"MPIE080406"						// game's id text, used in saved games - change this when you release a new version, to prevent old saved games to be loaded.

#def MAXCREDITS			3		// max number of credits
#def MAXCOINS			3		// max number of coins or diamonds to find (set it to the number of coins you added in your map)
#def MAXINVENTORY		3		// max number of inventory items ( max 10 items )
#def LOADINGTIME		3		// minimum number of seconds to wait on the loading page
...
#def PLAYER_BEGINX		600		// set the starting positon x of the player in map (when a new game begins)
#def PLAYER_BEGINY		358		// set the starting positon y of the player in map (when a new game begins)
